...
function App() {
  ...
  return (
    <div className="app">
      ...
      <footer className="status-bar" aria-label="실행 상태">
        <div className="status-bar-inner">
          <span className="status-item">
            <span className={`status-dot ${hasEnvKey || apiKey.trim() ? 'status-dot--ready' : ''}`} />
            {activeProvider}
          </span>
          <span className="status-item">
            {hasEnvKey ? '.env 키 연결됨' : apiKey.trim() ? '키 입력됨' : '키 필요'}
          </span>
          <span className="status-item">컴포넌트 {components.length}개</span>
        </div>
      </footer>
    </div>
  );
